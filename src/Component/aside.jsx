...
const Aside = () => {
  ...
  useEffect(() => {
    // Try to retrieve user data from sessionStorage
    const userData = JSON.parse(sessionStorage.getItem('user'));

    if (userData) {
      setUserName(userData.name || 'User');
      setUserProfilePicture(userData.profilePicture || null);
    } else {
      setUserName('User');
  ...
  // Generate initials for the placeholder image if no profile picture is available
  const userInitial = userName ? userName.charAt(0).toUpperCase() : 'U';
  const placeholderImage = `https://ui-avatars.com/api/?name=${encodeURIComponent(userInitial)}&size=40&background=random&color=fff`;
    ...
    <>
      <div className="w-[17.7%] h-[50vh] border-r-[0] flex justify-center mt-[8%] fixed max-sm:ml-[%] max-sm:mt-[37%]">
        <div className="flex justify-center flex-col items-start space-y-2 max-sm:gap-[10px]">
          ...
          <div className="flex items-center justify-center w-[40px] h-[40px] rounded-full max-sm:ml-[-5px]">
            {userProfilePicture ? (
              <img
